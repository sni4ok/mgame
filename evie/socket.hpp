...
int socket_accept_async(uint32_t port, const mstring& possible_client_ip, bool sync = false,
...
struct socket_stream
{
    ...
    socket_stream(uint32_t timeout, char_it buf, uint32_t buf_size, int s, socket_stream_op* op = nullptr);
    socket_stream(uint32_t timeout, char_it buf, uint32_t buf_size, const mstring& host,
        uint16_t port, socket_stream_op* op = nullptr);
    int recv();
    ~socket_stream();
    void compact();
    bool get(char& c);
...
struct udp_socket
{
    ...
    udp_socket(const mstring& host, uint16_t port, const mstring& src_ip = "", const mstring& ma = "");
    ~udp_socket();
};
