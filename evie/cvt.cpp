...
struct itoa_prealloc
{
    ...
    static const u32 values = pow10_v<digits>;
...
struct itoa_combined
{
    ...
    static const u64 values = pow10_v<digits>;
...
double lexical_cast<double>(char_cit from, char_cit to)
{
    ...
    if(p)
    {
        i64 v = atoi<i64>(from, d - from);
        u32 frac_sz = to - (d + 1);

        if(frac_sz > 19) [[unlikely]]
            throw mexception(es() % "lexical_cast<double>() error for: " % str_holder(from, to));

        u64 frac = atoi<u64>(d + 1, frac_sz);
        double f = double(frac);
        if(m)
            f = -f;
        return double(v) + f / __pow10[frac_sz];
    }
    if(e)
    {
        i64 mantissa = atoi<i64>(from, d - from);
        u64 exponent = atoi<u64>(d + 1, to - (d + 1));
