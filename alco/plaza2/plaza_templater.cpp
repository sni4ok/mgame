    ...
    void commit(context& ctx, bool end, char_cit i_f, char_cit i_t)
    {
        ...
        if(ctx.saved_namespace != ctx.cur_namespace)
        {
            ...
            else
            {
                ...
                if(!ctx.saved_namespace.second.empty())
                    ctx.sf << "}" << endl;
                ctx.sf << "namespace " << ctx.cur_namespace.first
                    << endl << "{ " << endl << "namespace "
        ...
        if(reply) {
            str << "        void print(mlog& ml) {"
                << endl <<  "            ml << \"" << ctx.table_name << "\"";
            for(const auto& v: types) {
                str << " << \", " <<  get<0>(v) << ": \" << " << get<0>(v);
            }
            str << ";" << endl << "        }" << endl;
        ...
        str << "    };" << endl;
        str << "    static_assert(sizeof(" << ctx.table_name << ") == "
            << structure_size << ", \"" << ctx.table_name << "\");" << endl;
        ...
        if(it == cur.end())
        {
            ...
            for(const auto& i : ctx.saved_structures)
            {
                ...
                {
                    found = true;
                    ctx.sf << "    using " << i.first.first << "::"
                        << i.first.second << "::" << ctx.table_name << ";" << endl;
                    break;
                }
            }
            if(!found)
        ...
        else
        {
            if(it->second != cur_str)
            {
                ...
                ctx.sf << "    //struct " << ctx.table_name
                    << " already defined early" << endl;
        }
        clear_and_close_namespace(ctx, end);
    }
};
...
mvector<mstring> list_folder(const mstring& ini_folder)
{
    ...
    while((dirp = readdir(dp.get())) != NULL)
    {
        ...
        if(dirp->d_type == DT_REG && fname.size() > 4
            && fname.back() == 'i' && *(fname.rbegin() + 1) == 'n'
...
void parse_all(const mstring& ini_folder, const mstring& out_file)
{
    ...
    for(const auto& v: unique_tables)
    {
        if(v.second != namespace_type())
            ctx.sf << "using " << v.second.first << "::"
                << v.second.second << "::" << v.first << ";" << endl;
    }
...
void proceed_selected(const mstring& ini_folder, const mstring& scheme,
    const mstring& out_folder)
{
        ...
        while(it != ie)
        {
            ...
            if(*it != ';')
            {
                ...
                std::copy(tables.begin(), tables.end(),
                    std::inserter(s->tables, s->tables.begin()));
                s->write_head(vs[0]);
